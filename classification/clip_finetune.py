"""
Fine-tune CLIP ViT-B/32 on folder-based image classification (Colab-friendly).

Expected layout:
  --train_dir (default /content/data/train): beach/, nature/, city/, culture/, festival/, food/
  --val_dir   (default /content/data/val):   beach/, nature/, city/, culture/, festival/, food/

Train and val must be separate folder trees (no shared image paths).
Final evaluation (zero-shot, fine-tuned, confusion matrices) uses val only — never train.
"""
...
def _assert_disjoint_splits(
    ...
    """Raise if the same image file appears in both train and val."""
    train_resolved = {p.resolve() for p in train_paths}
    val_resolved = {p.resolve() for p in val_paths}
    overlap = train_resolved & val_resolved
    if overlap:
        example = next(iter(overlap))
        raise ValueError(
            f"train and val must not share images ({len(overlap)} overlap). "
            f"Example: {example}"
        )
...
def _image_processor_settings(_processor: Any = None) -> Tuple[int, List[float], List[float]]:
    return _CLIP_IMAGE_SIZE, list(_CLIP_IMAGE_MEAN), list(_CLIP_IMAGE_STD)
...
def plot_zeroshot_vs_finetuned(
    zs_acc: float,
    ft_acc: float,
    out_path: Path,
) -> None:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Fine-tune CLIP on travel-style images.")
    parser.add_argument(
        "--train_dir",
        type=str,
    ...
    _assert_disjoint_splits(train_paths, val_paths)
    print(f"Train images: {len(train_paths)} | Val images: {len(val_paths)}")

    # Eval / zero-shot / confusion matrix: val only (no train leakage)
    eval_paths, eval_labels = val_paths, val_labels
    ...
    eval_ds = PILDataset(eval_paths, eval_labels, None)
    ...
    zs_acc, zs_y_true, zs_y_pred = zero_shot_accuracy(base, processor, eval_loader, device)
    ...
    te_loss, te_acc, y_true, y_pred = evaluate(model, eval_loader, device, loss_fn)
    print(f"Val (fine-tuned): loss={te_loss:.4f} acc={te_acc:.4f}")
    plot_confusion(
        zs_y_true,
        zs_y_pred,
        out_dir / "confusion_matrix_zeroshot.png",
        title="Zero-shot CLIP — val set",
    )
    plot_confusion(
        y_true,
        y_pred,
        out_dir / "confusion_matrix_finetuned.png",
        title="Fine-tuned CLIP — val set",
    )

    ft_acc = float(accuracy_score(y_true, y_pred))
    print("\n=== Zero-shot CLIP vs Fine-tuned CLIP (val set) ===")
    print(f"Zero-shot accuracy:  {zs_acc:.4f}")
    print(f"Fine-tuned accuracy: {ft_acc:.4f}")
    print(f"Δ (fine-tuned - zero-shot): {ft_acc - zs_acc:+.4f}")

    print("\nPer-class (fine-tuned, val):")
    ...
    plot_zeroshot_vs_finetuned(zs_acc, ft_acc, out_dir / "zeroshot_vs_finetuned.png")
