...
def explain(
    ...
    prompt = f"""당신은 친근한 여행 큐레이터입니다.
아래 CV 분석 결과를 바탕으로 사용자의 여행 취향을 설명해주세요.

반드시 아래 형식으로만 답변하세요 (다른 형식 절대 금지):

1. 여행 성향 설명 (2~3문장, 수치/퍼센트 언급 금지, 자연스러운 존댓말)
2. 추천 여행지 3곳:
   - [도시/나라]: 이유 한 문장
   - [도시/나라]: 이유 한 문장
   - [도시/나라]: 이유 한 문장
...
top_category별 말투 (recommendation.top_category 기준):
- food: 먹는 게 여행이다! 현지 맛집 탐방 스타일
- city: 도시의 에너지를 즐기는 스타일
- beach: 파도 소리가 그리운 바다파
- nature: 자연 속에서 쉬고 싶은 스타일
- culture: 진짜 그 나라를 느끼고 싶은 탐구형
- festival: 흥이 넘치는 축제파

추가 규칙:
- 수치, 퍼센트, 소수점 숫자 절대 출력 금지
- ** 같은 마크다운 기호 사용 금지
- 이모지는 추천 여행지 앞에만 허용
- 너무 짧게 답하지 말 것
